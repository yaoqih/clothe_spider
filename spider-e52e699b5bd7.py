...
class FARFETCHSpider(Spider):
    ...
    async def next_page_btn(self, page):
        ...
        link = next_page_btn.get_attribute('href')
        link = page.url + link
        return link
    ...
    async def items_in_page(self, page):
        ...
        assert block, ("未找到类别页面的 section[id='portal-slices-listing'], 可能以下原因：\n"
                       ...
                       "2. 404 页面，访问过多导致被限流；")
        items = await block.query_selector_all("li[data-component='ProductCard']")
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(
        spider.async_run(spider_map[platform][1], spider_map[platform][2], concurrency=concurrency, headless=False, next_page_click=False)
    )
